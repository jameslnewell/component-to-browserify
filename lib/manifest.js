...
function dependency(oldDep, oldVersion, callback) {
  client()
    ...
    .get('https://api.github.com/repos/'+oldDep+'/contents/package.json', function(err, res) {
      ...
      var manifest = JSON.parse(new Buffer(res.getBody().content, 'base64').toString())
...
function dependencies(component, oldDeps, callback) {
  var i= 0, repos = Object.keys(oldDeps), newDeps = {};//TODO: browserify options

  function next() {

    if (i>=repos.length) {
      return callback(null, newDeps);
    }

    dependency(repos[i], oldDeps[repos[i]], function(err, newDep, newVersion) {
      if (err) return callback(err);
      ++i;
      newDeps[newDep] = newVersion;
      next();
    });
  }
